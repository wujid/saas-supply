<template>
  <div v-if="item.businessStatus === 1">
    <template v-if="hasOneShowingChild(item.childrenList,item) && (!onlyOneChild.childrenList||onlyOneChild.noShowingChildren)&&!item.alwaysShow">
      <app-link v-if="onlyOneChild" :to="resolvePath(onlyOneChild.path)">
        <el-menu-item :index="resolvePath(onlyOneChild.path)" :class="{'submenu-title-noDropdown':!isNest}">
          <item :icon="onlyOneChild.icon||(item&&item.icon)" :title="onlyOneChild.name" />
        </el-menu-item>
      </app-link>
    </template>
